...
//now, only non-tuning mode works. Should remove all tuning-related codes if not used in final version.
namespace QoZ {
    template<class T, uint N, class Quantizer, class Quantizer_EB, class Encoder, class Lossless>
    class SZQoIInterpolationCompressor : public concepts::CompressorInterface<T> {//added heritage
    public:
        ...
        void build_grid(Config &conf, T *data,size_t maxStep,int tuning=0){
            ...
            if (N==1){
                for (size_t x=maxStep*(tuning==1);x<conf.dims[0];x+=maxStep){
                    ...
                    quantizer.insert_unpred(*(data+idx));
                    quant_inds[quant_index]=0;
                    quant_inds[quant_index+num_elements]=0;
                    quant_index++;
...
//            printf("%.8f \n", max_error);
        ...
        size_t quant_index = 0; // for both compress and decompress
        size_t maxStep=0;
        //double max_error;
        Quantizer quantizer;
